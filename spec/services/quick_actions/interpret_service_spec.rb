...
describe QuickActions::InterpretService do
  ...
  describe '#execute' do
    ...
    shared_examples 'milestone command' do
      ...
      it 'returns and empty miestone message when milestone is wrong' do
    ...
    shared_examples 'done command' do
      ...
      it 'returns the done message' do
        ...
        expect(message).to eq('Marked todo as done.')
      end
    end
    ...
    shared_examples 'due command' do
      ...
      it 'returns due_date message: Date.new(2016, 8, 28) if content contains /due 2016-08-28' do
        ...
        expected_date_text = (defined?(expected_date) ? expected_date : Date.new(2016, 8, 28)).to_s(:medium)
        expect(message).to eq("Set the due date to #{expected_date_text}.")
      end
    end
    ...
    describe 'move issue command' do
      it 'returns the move issue message' do
        _, _, message = service.execute('/move test/project', issue)

        expect(message).to eq('Moved this issue to test/project.')
      end
    end
